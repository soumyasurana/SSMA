...
class NewSaleScreen extends StatefulWidget {
  ...
  const NewSaleScreen({super.key, this.existingSale});
...
class _NewSaleScreenState extends State<NewSaleScreen> {
  ...
  void _showQuantityDialog(Product product) {
    ...
    final priceController = TextEditingController(text: product.salePrice.toStringAsFixed(2));
  ...
  Future<void> _completeSale() async {
    ...
    if (_selectedSaleType == SaleType.cash && (buyerName.isEmpty || buyerContact.isEmpty)) {
      ScaffoldMessenger.of(context).showSnackBar(
    ...
    if (widget.existingSale != null) {
      newSale.id = widget.existingSale!.id;
    }
    ...
    for (final item in _saleItems) {
      ...
      if (matchingProduct.id != -1) {
        ...
        if (projectedQty < 0) {
          warningItems.add('${item.productName} (will be ${projectedQty.toStringAsFixed(2)})');
        }
      }
    }
    ...
    await DBService.recordSale(newSale);
    ...
    for (final item in _saleItems) {
      ...
      if (matchingProduct.id != -1) {
        matchingProduct.quantity -= item.quantity.toInt();
        await DBService.updateProduct(matchingProduct);
      }
    }
          ...
          child: Column(
            ...
            children: [
              ...
              if (_selectedSaleType == SaleType.cash) ...[
                ...
                const SizedBox(height: 10),
                _buildTextField(_buyerContactController, 'Buyer Contact', isPhone: true),
              ] else ...[
